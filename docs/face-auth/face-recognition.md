...
```mermaid
flowchart TD
  frame["SharedFrame から最新フレーム取得<br>(3秒より古ければエラー)"] --> det["顔検出 SCRFD(det_10g)"]
  det -- "顔なし" --> clear["オーバーレイ消去・未検出で返す"]
  det -- "最高スコアの顔" --> gate{"顔幅比率 ≥ minFaceWidthRatio ?"}
  gate -- "No" --> bboxonly["bbox のみ返す(照合スキップ)"]
  gate -- "Yes" --> lmk["106点ランドマーク(2d106det)<br>→ アライメント5点(失敗時は SCRFD の5点)"]
  lmk --> embed["ArcFace(w600k_r50)で<br>512次元 embedding 抽出"]
  embed --> match["登録済み全件と 1:N コサイン類似度"]
  match --> decide{"1位 ≥ matchThreshold かつ<br>1位-2位 ≥ matchMargin ?"}
  decide -- "Yes" --> ok["recognized = true(userId 確定)"]
  decide -- "No" --> ng["該当者なし"]
```
...
3. **顔サイズゲート** — 顔幅がフレーム幅の `minFaceWidthRatio`(既定 0.22)未満なら、
   ランドマーク・embedding・照合を省略してbboxだけを返す(使われない結果へ高コストな
   推論をかけない)。フロントは同じ `minFaceWidthRatio` を近接判定(下記「近づいて
   ください」)にも使うため、Rust とフロントで同一のしきい値になる。
...
- **連続一致ガード**: 同一人物が `recognitionStableCount`(既定 1)回連続で認識されたときだけ
  確認カードを表示する。別人・非検出でカウントはリセット。有効な最小値は1で、
  保存済みの0は1として扱い、1回の一致で確認カードを表示する。
- 顔幅比率が `minFaceWidthRatio`(既定 0.22、設定のパフォーマンス項目で調整可)未満の
  うちは「もう少し近づいてください」を表示。値を下げるほど遠くても認証を試みる。
- 確認カード表示中はSCRFDの顔検出だけを実行し、ランドマーク・embedding・照合を
  やり直さない。2回(`MISS_STREAK_TO_DISMISS`)連続で顔が検出されなければ離れたと
  みなしてカードを自動で閉じる。否認(「ちがう」・サムズダウン)した場合のみ、
  一度顔が外れるまで再確定を抑止する。記録完了・確定後は顔の離脱を待たず、同じ人物が
  フレームに残っていても連続一致カウントを0から取り直し、設定回数に達すると再び確認カードを表示する。
...
- 顔枠は未認識=シアン / 認識成功=エメラルドで色分けする。手は骨格線+関節点。
  確認カード表示中は照合を省略する(`matchFaces=false`)ため素の判定は常に
  「未認識」になるが、フロントが `overlayRecognized=true` を渡して**本人特定済みの
  緑の枠を維持する**(カードが出ているのに枠がシアンへ戻る、を防ぐ)。
...
実行される。顔認証とジェスチャーは個別にも初期化でき、一方のモデル破損で他方まで
停止しない。モデルパスの解決規約は `src-tauri/src/vision/paths.rs` を参照。
