...
- **実装方式**: アプリのアクセントは Tailwind の cyan 系ユーティリティ
  ...
  この変数群を別パレットへ丸ごと差し替えることで、ボタン・見出し・グローなど
  React / CSS で描画する UI の差し色が一括で切り替わる。
- Rust がカメラフレームへ焼き込む顔枠・顔ランドマーク・手の骨格は、映像上での
  視認性を優先した固定色であり、アクセントカラーには追従しない。顔枠は
  未認識=シアン / 認識成功=エメラルドで表示する。
