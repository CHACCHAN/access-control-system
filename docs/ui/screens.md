...
- ヘッダー: 顔登録モード切替・テーマ切替・**在室状況の更新**・
  **外部サイト(地球儀)**・設定ページを開くボタン(この並び順)。
  更新ボタンは Socket.IO の通知が届かない場合の予備手段で、押すとメンバー一覧を
  取り直す(取得中はアイコンが回転し、二重実行はできない)。
  アイコンボタンの見た目は共通部品 `shared/ui/IconButton` に集約している。
- アイコンだけのボタンは、**マウスホバー(およびキーボードフォーカス)で
  ボタンの下に用途のラベルを表示**する(例: 設定ボタン →「設定を開く」)。
  ラベルは `IconButton` の `label`(= `aria-label`)と同じ文字列を使う。
  タッチ操作を妨げないよう `pointer-events-none`、読み上げの二重化を避けるため
  ツールチップ自体は `aria-hidden`。
