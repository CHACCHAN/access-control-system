 ...
 */
export function GestureGuide({ detectedGesture, title, includeReject = false }: GestureGuideProps) {
  ...
  const chipClass = (active: boolean, reject: boolean) =>
    ...
    <div className="rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-3 dark:border-white/10 dark:bg-slate-950/40">
      ...
      <div className="mt-2 flex flex-wrap justify-center gap-3">
        {assigned.map(({ key, kind, icon: Icon }) => (
          <span key={key} className={chipClass(detectedGesture === kind, false)}>
            <Icon className="h-5 w-5" />
            {settings.gestureStatusMap[key]}
          </span>
        ))}
        {showReject && (
