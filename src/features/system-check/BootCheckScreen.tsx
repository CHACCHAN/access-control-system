  ...
  return (
    <div className="fixed inset-0 z-100 flex flex-col items-center justify-center bg-black px-6 font-mono text-sm text-slate-300">
      <div className="w-full max-w-md">
        <p className="mb-4 text-slate-500">在室管理システムを起動しています...</p>

        <ul className="space-y-1.5">
          {BOOT_CHECKS.map((check) => {
            const state = results[check.id];
            return (
              <li key={check.id} className="flex gap-3">
                <span className={STATE_CLASSES[state]}>[{STATE_LABEL[state]}]</span>
        ...
        {isComplete && hasFailure && (
          <div className="mt-6 animate-fade-in">
            <p className="text-amber-400">一部のチェックに失敗しました。続行しますか？</p>
            <button
              onClick={onContinue}
