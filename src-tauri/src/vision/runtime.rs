...
pub fn init_onnxruntime(dylib_path: &PathBuf) -> Result<(), String> {
    ORT_INIT
        .get_or_init(|| {
...
pub fn load_session(label: &str, path: &Path) -> Result<Session, String> {
    let started = Instant::now();
    ...
    .map_err(|e| format!("モデル {label} のロードに失敗しました ({}): {e}", path.display()))?;
    eprintln!(
...
pub fn run_single_input(
    ...
    let tensor =
        ort::value::Tensor::from_array(input).map_err(|e| format!("入力テンソル生成に失敗: {e}"))?;
    let outputs = session
